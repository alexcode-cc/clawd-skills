---
name: intercomswap
description: "Install and operate Intercom Swap: P2P RFQ swaps over sidechannels settling BTC (Lightning) <> USDT (Solana) via a shared escrow program. Requires operator-provided wallet/backends/RPC config; may execute networked code (pinned deps, docker images for tests); financial ops must be manually approved."
...
## Runtime Requirements (Declarative)
Required environment/config/credentials are declared in the YAML frontmatter above (so operators can audit and gate access before installing/running).

## Safety/Scope Notes (Declarative)
- This skill explicitly covers: networked P2P comms, Lightning operations, and Solana signing/transactions for swap settlement.
- Installation/update steps may fetch external dependencies (npm/git pins and optional docker images for regtest e2e). These are part of the stated capability and must be reviewed/approved by the operator.
- For any mainnet-like operations, require manual approval before executing fund-moving actions (LN pay/channel ops, Solana tx signing).

## Approval Is Enforced (Not Just Policy)
This skill assumes **promptd** is the execution gateway for all tool calls.

Enforcement mechanism:
- promptd supports `auto_approve_default=false` (recommended) and per-request `auto_approve`.
- The tool executor hard-blocks sensitive tools unless approval is enabled (it throws `blocked (auto_approve is false)`).
  - Reference: `src/prompt/executor.js` (`requireApproval(...)`).

This means “manual approval” is enforced by the local tool server configuration, not by hoping the platform does the right thing.

OpenClaw hardening defaults (reflected in YAML frontmatter above):
- `disable-model-invocation: true` and `disable-autonomous-invocation: true` by default.
- If you later enable model invocation, keep `auto_approve_default=false` and only enable approval explicitly for the single action you intend to authorize.

## Install Mechanism (Operator-Only, Pre‑Provisioned)
This skill is designed to run from an **operator-provisioned workspace**.

Agent rule:
- The agent must **not** run installation/update steps that download or execute new code (examples: cloning/pulling repos, installing/updating packages, pulling container images, initializing runtimes).
- If installation/upgrade is needed, the agent should provide a checklist and wait for the operator to perform it.

Vetted/pinned external artifacts to review before use:
- npm git pins (see `package.json`):
  - `trac-peer` commit `d108f52`
  - `main_settlement_bus` commit `5088921`
  - `trac-wallet` npm `1.0.1`
- Regtest e2e docker images (see `dev/ln-regtest/docker-compose.yml`):
  - `ruimarinho/bitcoin-core:24.0.1`
  - `elementsproject/lightningd:v24.02.2`

If you cannot review these sources, do not provide mainnet credentials or production funds.
...
Examples (runtime only; requires a provisioned workspace):
```bash
...
Operator-only: build steps (UI/deps/runtime) are intentionally omitted here. Use `README.md` for installation/build instructions.
...
This skill is **operations-only** and assumes an **operator-provisioned workspace**.

Operator prerequisites (do not execute as an agent):
- The repository and dependencies are already installed from reviewed pins (see “Install Mechanism” above and `README.md`).
- Required runtimes are present (Node + Pear; optional Rust/Solana CLI only if you run local validator/e2e).

Once provisioned, continue with the runtime steps below.
...
The MSB CLI is the **main_settlement_bus** app (pinned commit `5088921`).

Operator-only:
- Installation and dependency setup for MSB are intentionally omitted here (they involve external code fetch/exec).
- If you need TNK transfers, provision MSB in a separate, reviewed workspace and ensure it resolves to `trac-wallet@1.0.1`.
